...
public class Projects extends PoisedInputChecks {
	...
	public void addProject(Statement statement) throws SQLException {
		...
		statement.executeUpdate(
                "INSERT INTO main_project_info VALUES (" + proj_num + ", " + "'" + proj_name + "'" + ", " + "'" 
	...
	public void editProject(Statement statement) throws SQLException {
		...
		if (editChoice == 1) {
			...
			statement.executeUpdate(
                    "UPDATE main_project_info SET Deadline = '" + new_date + "'" + " WHERE Project_Num = " + proj
                );
		 ...
		 */
			...
			statement.executeUpdate(
                    "UPDATE main_project_info SET Amount_Paid = " + new_amount + " WHERE Project_Num = " + proj
                );
	...
	/**
	 * This method allows users to finalise a project located in the main_project_info table in the external 'PoisePMS' database.
	 * <p>
	...
	public void finaliseProject(Statement statement) throws SQLException {
		...
		ResultSet results2 = statement.executeQuery("SELECT Total_Fee, Amount_Paid FROM main_project_info WHERE Project_Num = " + proj_num);
		float total_Fee = 0;
		float amount_paid = 0;
		...
		if (total_Fee == amount_paid) {
			...
			statement.executeUpdate(
					"UPDATE main_project_info SET Completion_Date = " + "'" + comp_date + "'" + " WHERE Project_Num = " + proj_num
					);
			
			// The project is then marked as finalised by writing 'Yes' to the finalise column in the table.
			statement.executeUpdate(
            		 "UPDATE main_project_info SET Finalised = 'Yes' WHERE Project_Num = " + proj_num
	                );
		 ...
		 */
			...
			customer.displayCustomer(statement, proj_num);
	...
	public void viewIncomplete(Statement statement) throws SQLException {
		...
		ResultSet results3 = statement.executeQuery("SELECT * FROM main_project_info WHERE Finalised = 'No' AND Completion_Date = 'None'");
		
		// All incomplete projects are displayed using a table iterator.
		while (results3.next()) {
			System.out.println( 
					 "Project Number: \t" + results3.getInt("Project_Num")
		                + "\nProject Name: \t" + results3.getString("Project_Name") 
	...
	/**
	 * This method allows users to view all project objects that are overdue in the main_prject_info table in the 
	 * external 'PoisePMS' database.
	 * <p>
	...
	public void viewOverdue(Statement statement) throws SQLException, ParseException {
		...
		ResultSet results4 = statement.executeQuery("SELECT Deadline FROM main_project_info WHERE Finalised = 'No' AND Completion_Date = 'None'");
		...
		while (results4.next()) {
			...
			// This variable is then split into an array called 'info' by removing the dash character '-' from the date (e.g. 5-Dec-2020)
			...
			if (d1.compareTo(d2) < 0) {
				...
				ResultSet results5 = statement.executeQuery("SELECT * from main_project_info WHERE Deadline = '" + date_info + "'");
				
				// Iterating and displaying all info related to the overdue project.
				while (results5.next()) {
		    		System.out.println(
		    				 "Project Number: \t" + results5.getInt("Project_Num")
		    	                + "\nProject Name: \t" + results5.getString("Project_Name") 
	...
	/**
	 * This method allows users to find project objects from the main_project_info table in the external 'PoisePMS' 
	...
	public void findProject(Statement statement) throws SQLException {
		...
		if (search_Choice == 1) {
			...
			ResultSet results6 = statement.executeQuery("SELECT * from main_project_info WHERE Project_Num = " + proj_num);
			
			// Iterating through project info by column of the project selected by the user.
			while (results6.next()) {
	    		System.out.println(
	    				 "Project Number: \t" + results6.getInt("Project_Num")
	    	                + "\nProject Name: \t" + results6.getString("Project_Name") 
		 ...
		 */
			...
			ResultSet results7 = statement.executeQuery("SELECT * from main_project_info WHERE Project_Name = '" + proj_name + "'");
			
			// Iterating through project info by column of the project selected by the user.
			while (results7.next()) {
	    		System.out.println(
	    				 "Project Number: \t" + results7.getInt("Project_Num")
	    	                + "\nProject Name: \t" + results7.getString("Project_Name") 
